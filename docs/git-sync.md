...
1. **Review + Push** – Use the cloud-upload icon in the header (or Settings card) after making changes on one device. GTD Space first saves open tabs, shows a diff preview against the latest local encrypted backup, and only encrypts/commits after you confirm.
2. **Pull** – On another device, click the cloud-download icon. The app fetches latest backups, decrypts, and replaces the local workspace.
...
- **Binary files** show metadata such as path, size, MIME guess, and SHA-256 fingerprints.
