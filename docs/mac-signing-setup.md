...
This guide documents the macOS signing setup used by the current release workflow in [`docs/release-process.md`](./release-process.md).

> Configure the secrets below, then push a release tag or run the release workflow. GitHub Actions imports the Developer ID certificate, signs `GTD Space.app`, and notarizes the DMG automatically.
...
| `TAURI_SIGNING_PRIVATE_KEY` / `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` | Already used by the updater signing flow | Separate from macOS notarization |

In `.github/workflows/build.yml`, the workflow maps these secrets to the environment variables expected by `tauri-action`:

- `CSC_LINK` -> `APPLE_CERTIFICATE`
- `CSC_KEY_PASSWORD` -> `APPLE_CERTIFICATE_PASSWORD`
- `APPLE_ID` -> `APPLE_ID`
- `APPLE_APP_SPECIFIC_PASSWORD` -> `APPLE_PASSWORD` and `APPLE_ID_PASSWORD`
- `APPLE_TEAM_ID` -> `APPLE_TEAM_ID`
- `MACOS_KEYCHAIN_PASSWORD` -> used when creating the temporary keychain
- `TAURI_SIGNING_PRIVATE_KEY` and `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` -> passed through unchanged for updater signing
...
Within `.github/workflows/build.yml` the macOS runners:

1. Import the certificate into a temporary keychain using `CSC_LINK`, `CSC_KEY_PASSWORD`, and `MACOS_KEYCHAIN_PASSWORD`.
2. Discover the Developer ID identity and expose it as `APPLE_SIGNING_IDENTITY`.
...
If any required secret is missing, the macOS build jobs fail fast with a clear error.
