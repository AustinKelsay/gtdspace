...
if (!['major', 'minor', 'patch'].includes(versionType) && !/^\d+\.\d+\.\d+$/.test(versionType)) {
  exitWithError(`Invalid version type: ${versionType}. Use major, minor, patch, or x.y.z`);
}
...
if (!remoteCommit) {
  exitWithError(`Remote branch origin/${currentBranch} not found`);
}
...
try {
  execCommand(`npm run version:${versionType}`);
