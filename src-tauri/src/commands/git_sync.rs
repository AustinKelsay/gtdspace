...
pub fn compute_git_status(
    ...
    if configured {
                ...
                if message.is_none() {
                    match run_git_command(&repo_buf, ["remote"]) {
                        Ok(remotes) => {
                            has_remote = remotes
                                .lines()
                                .any(|line| line.trim() == REMOTE_NAME);
                        }
                        Err(_) => {
                            has_remote = false;
                        }
        ...
        message = Some(
            "Git sync requires repo path, workspace path, and encryption key".to_string(),
        );
    } else {
...
pub fn build_git_sync_config(
    ...
    if repo_real.starts_with(&workspace_real) || workspace_real.starts_with(&repo_real) {
        return Err(
            "Repository path must be outside the workspace to avoid recursive backups"
                .to_string(),
        );
    }
...
fn ensure_gitignore(repo_path: &Path) -> Result<(), String> {
    ...
    if !gitignore_path.exists() {
        ...
        for line in desired { contents.push_str(line); contents.push('\n'); }
