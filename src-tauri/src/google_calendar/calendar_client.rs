...
pub async fn fetch_calendar_events(access_token: &str) -> Result<Vec<CalendarEvent>, Box<dyn std::error::Error>> {
    ...
    println!("[CalendarClient] Time range: {} to {}", time_min.to_rfc3339(), time_max.to_rfc3339());
    ...
    if !response.status().is_success() {
        ...
        return Err(format!("Failed to fetch events: {}", error_text).into());
    }
...
pub async fn fetch_events_async(access_token: &str) -> Result<Vec<CalendarEvent>, Box<dyn std::error::Error + Send + Sync>> {
